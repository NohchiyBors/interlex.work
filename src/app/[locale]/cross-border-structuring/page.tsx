...
import { getStructuringIntentSection, getStructuringScopeSection } from "@/lib/seo-page-sections";
...
export async function generateMetadata(props: PageProps<"/[locale]/cross-border-structuring">) {
  const { locale } = await props.params;
  if (!hasLocale(locale)) notFound();
  const content = getCrossBorderStructuringContent(locale);

  return {
    ...buildMetadata(locale, content.title, content.description, "cross-border-structuring"),
    keywords: content.keywords,
  };
}

export default async function CrossBorderStructuringPage(props: PageProps<"/[locale]/cross-border-structuring">) {
  const { locale } = await props.params;
  if (!hasLocale(locale)) notFound();
  const content = getCrossBorderStructuringContent(locale);
  ...
  const intentSection = getStructuringIntentSection(locale);
  const scopeSection = getStructuringScopeSection(locale);
    ...
    <main className="mx-auto w-full max-w-7xl px-6 pb-14 pt-8 md:px-8 xl:px-12 xl:pt-12">
      <JsonLd
        data={[
          getBreadcrumbJsonLd([
            ...
            { name: content.title, url: `https://interlex.work${localePath(locale, "cross-border-structuring")}` },
          ]),
      ...
      <section className="border border-[color:rgba(0,9,36,0.08)] bg-white px-6 py-8 shadow-sm md:px-8 md:py-10">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          <Link
            href={localePath(locale, "kazakhstan-vs-georgia")}
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <article className="border border-[color:rgba(0,9,36,0.08)] bg-[var(--surface-low)] px-6 py-7">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--accent)]">{content.scenariosLabel}</p>
          <h2 className="mt-3 font-display text-4xl leading-none text-[var(--primary)]">{content.scenariosTitle}</h2>
        ...
        <article className="border border-[color:rgba(0,9,36,0.08)] bg-white px-6 py-7">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--accent)]">{content.risksLabel}</p>
          <h2 className="mt-3 font-display text-4xl leading-none text-[var(--primary)]">{content.risksTitle}</h2>
          <p className="mt-4 text-sm leading-7 text-[var(--muted)]">{content.risksBody}</p>
        </article>
      </section>
      ...
      <section className="mt-6 border border-[var(--primary)] bg-[var(--primary)] px-6 py-8 text-white md:px-8">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[rgba(220,192,162,0.88)]">{content.whyLabel}</p>
        <div className="mt-4 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="max-w-3xl">
            <h2 className="font-display text-4xl leading-none tracking-[-0.04em] text-white">{content.whyTitle}</h2>
            <p className="mt-4 text-sm leading-7 text-white/78">{content.whyBody}</p>
          </div>
      ...
      <section className="mt-6 border border-[color:rgba(0,9,36,0.08)] bg-white px-6 py-8 shadow-sm md:px-8">
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          {[comparisonPage, marketEntryPage].map((page) => (
            <Link
              key={page.slug}
